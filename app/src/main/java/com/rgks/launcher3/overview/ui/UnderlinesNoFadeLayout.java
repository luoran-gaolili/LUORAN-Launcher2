...
public class UnderlinesNoFadeLayout extends LinearLayout implements InitPage {
    ...
    //单例模式获取实例
    public static UnderlinesNoFadeLayout getUnderLinesSingleInstance(Context context) {
        if (underLinesSingleInstance == null) {
            synchronized (UnderlinesNoFadeLayout.class) {
                if (underLinesSingleInstance == null) {
                    underLinesSingleInstance = new UnderlinesNoFadeLayout(context);
                }
            }
        }
        return underLinesSingleInstance;
    }
    ...
    public void setIsFirstDone(Boolean flag) {
        mIsFirstDone = flag;
